function [sys, x0, str, ts] = Nepsac_DeltaU_UBFilter3(t,x,u,flag,ts)
...
elseif flag == 3
    ...
    while (aux > ro)
        ...
        % reference errors
        xerro = [xr(k) xr(k+2) xr(k+5) xr(k+9) xr(k+14)];
        yerro = [yr(k) yr(k+2) yr(k+5) yr(k+9) yr(k+14)];
        thetaerro = [tr(k) tr(k+2) tr(k+5) tr(k+9) tr(k+14)];
